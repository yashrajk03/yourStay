...
const Navbar = () => {
  ...
  // Handle scroll and location changes
  useEffect(() => {
    const isNotHome = location.pathname !== '/';
    const handleScroll = () => {
      setIsScrolled(isNotHome || window.scrollY > 10);
    };
    handleScroll(); // run on mount & route change
        ...
        {user ? (
          <>
            <button
              ...
              className="flex items-center gap-2 border px-4 py-2 rounded-full text-sm text-gray-700"
            >
              <BookIcon />
              My Bookings
            </button>
